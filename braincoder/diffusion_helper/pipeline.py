...
def generate(embedding, vae: AutoencoderKL, unet: UNet2DConditionModel, scheduler: LMSDiscreteScheduler, device, num_inference_steps=100, guidance_scale=7.5, seed=0):
...
def get_emb(brain_embedding, device):
    if brain_embedding.size().__len__() == 2:
...
def brain2image(z, vae, unet, scheduler, steps, device):
    ...
    generated_images = generate(embedding, vae=vae, unet=unet, scheduler=scheduler, device=device, num_inference_steps=steps)
